...
(* 
following coder are modified 
from register.ml provided by professor
*)
...
let load sym thestack = 
    let result,_ = aget symbol_table sym
    in let _,value = aget symbol_table sym 
    in if result = true
    then push (value) thestack
    else printf "register '%d' () empty\n" sym    
